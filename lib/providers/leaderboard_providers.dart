...
final leaderboardProvider =
    FutureProvider.autoDispose<List<LeaderboardStanding>>((ref) async {
      ...
      final standings = (response as List)
          .map(
            (json) =>
                LeaderboardStanding.fromJson(json as Map<String, dynamic>),
          )
          .toList();
...
final competitorTeamDetailsProvider = FutureProvider.autoDispose
    .family<UserTeam?, String>((ref, teamId) async {
      final client = ref.watch(supabaseClientProvider);
...
final competitorTeamProvider = AsyncNotifierProvider.autoDispose
    .family<CompetitorTeamNotifier, List<TeamHoleScore>, String>((arg) {
...
class CompetitorGolfersHoleScoresNotifier
    extends AsyncNotifier<List<HoleScore>> {
  final String teamId;
  final int round;
  ...
  FutureOr<List<HoleScore>> build() async {
    final client = ref.watch(supabaseClientProvider);
    final teamAsync = await ref.watch(
      competitorTeamDetailsProvider(teamId).future,
    );
...
final competitorGolfersHoleScoresProvider = AsyncNotifierProvider.autoDispose
    .family<
      CompetitorGolfersHoleScoresNotifier,
      List<HoleScore>,
      ({String teamId, int round})
    >((arg) {
...
final competitorGolfersTeeTimesProvider = FutureProvider.autoDispose
    .family<List<TeeTime>, ({String teamId, int round})>((ref, arg) async {
      final client = ref.watch(supabaseClientProvider);
...
final competitorScorecardSubscriptionProvider = Provider.autoDispose
    .family<void, ({String teamId, int round})>((ref, arg) {
      final client = ref.watch(supabaseClientProvider);
      final teamAsync = ref.watch(competitorTeamDetailsProvider(arg.teamId));
      final team = teamAsync.value;
      if (team == null) return;
      ...
      final channelName =
          'competitor-scorecard-realtime-${arg.round}-${team.id}';
      final channel = client.channel(channelName);
            ...
            callback: (payload) {
              ...
              if (record.isNotEmpty) {
                ...
                if (golferId != null &&
                  ...
                  ref
                      .read(competitorGolfersHoleScoresProvider(arg).notifier)
                      .updateScore(newScore);

                  // Get the latest golfer scores to update team best ball
                  final updatedGolferScores =
                      ref
                          .read(competitorGolfersHoleScoresProvider(arg))
                          .value ??
                      [];
                  ref
                      .read(competitorTeamProvider(arg.teamId).notifier)
                      .updateBestBall(
                        arg.round,
                        newScore.hole,
                        newScore.par,
